...
void cmd::area(const std::vector< Polygon >& polygons, std::istream& in, std::ostream& out)
{
  ...
  out << std::accumulate(
    ...
    [&resultFuncForArea](double sum, const Polygon& polygon) -> double { return sum + resultFuncForArea(polygon); }
  );
}
...
void cmd::count(const std::vector< Polygon >& polygons, std::istream& in, std::ostream& out)
{
  ...
  out << std::accumulate(
    ...
    [&resultFuncForCount](double sum, const Polygon& polygon) -> double { return sum + resultFuncForCount(polygon); }
  );
}
...
double cmd::subcmd::getPolygonArea(const Polygon& polygon)
{
  ...
  using namespace std::placeholders;
  std::transform(
    polygon.points.begin() + 2, polygon.points.end(), std::next(polygon.points.begin(), 1), triangleAreas.begin(),
...
double cmd::subcmd::getMaxPolygonArea(const std::vector< Polygon >& polygons)
{
  if (polygons.empty())
  {
    throw std::invalid_argument("for working MAX AREA need more one figure");
  }
  using namespace std::placeholders;
  auto maxIt = std::max_element(
    polygons.begin(), polygons.end(),
    std::bind(std::less< double >{}, std::bind(getPolygonArea, _1), std::bind(getPolygonArea, _2))
  );
...
double cmd::subcmd::getMinPolygonArea(const std::vector< Polygon >& polygons)
{
  if (polygons.empty())
  {
    throw std::invalid_argument("for working MIN AREA need more one figure");
  }
  using namespace std::placeholders;
...
size_t cmd::subcmd::getMaxPolygonVertexes(const std::vector< Polygon >& polygons)
{
  ...
  auto maxIt = std::max_element(
    polygons.begin(), polygons.end(),
    std::bind(
      std::less< size_t >{}, std::bind(&std::vector< Point >::size, std::bind(&Polygon::points, _1)),
...
bool cmd::subcmd::isHasRightAngle(const Polygon& polygon)
{
  ...
  auto findRightAngle =
    RightAnglePred{ polygon.points[polygon.points.size() - 2], polygon.points[polygon.points.size() - 1] };
