...
double cmd::subcmd::getPolygonArea(const Polygon& polygon) {
  ...
  using namespace std::placeholders;
  std::transform(polygon.points.begin() + 2, polygon.points.end(),
...
size_t cmd::subcmd::getMaxPolygonVertexes(const std::vector<Polygon>& polygons) {
  ...
  auto maxIt = std::max_element(
    ...
    [](const Polygon& a, const Polygon& b) -> bool { return a.points.size() < b.points.size(); });
...
bool cmd::subcmd::isHasRightAngle(const Polygon& polygon) {
  ...
  auto findRightAngle = RightAnglePred{ polygon.points[polygon.points.size() - 2],
                                       polygon.points[polygon.points.size() - 1] };
  return (std::find_if(polygon.points.cbegin(), polygon.points.cend(), findRightAngle) !=
    polygon.points.cend());
}
